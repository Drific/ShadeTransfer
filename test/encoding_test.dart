...
void main() {
  ...
  group('Base-55 encoding', () {
    ...
    test('round-trip for random strings', () {
      for (int i = 0; i < 500; i++) {
        final original = EncodingService.randomString(1 + (i % 20));
  ...
  group('ICE candidate encoding', () {
    test('round-trip for typical LAN address', () {
      final encoded = EncodingService.encodeIceCandidate('192.168.1.100', 8080);
    ...
    test('round-trip for port range boundaries', () {
      ...
      final high = EncodingService.encodeIceCandidate('192.168.255.254', 8150);
    ...
    test('encoded result contains no forbidden characters', () {
      for (int p = 8000; p <= 8150; p += 7) {
        final encoded = EncodingService.encodeIceCandidate('192.168.1.100', p);
